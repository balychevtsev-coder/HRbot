...
class Form(StatesGroup):
    ...
    waiting_for_vacancy_docx = State()
    waiting_for_multi_resumes = State()
...
Ты — старший HR-архитектор. Твоя цель: синтезировать эталонный профиль вакансии на основе предоставленных резюме.

ПРАВИЛА ОФОРМЛЕНИЯ (КРИТИЧЕСКИ ВАЖНО):
1. ЗАПРЕЩЕНО выводить символы ### и двойные кавычки " в итоговом тексте. Это служебные знаки только для этого промпта.
2. Текст, который в шаблоне находится между ###, выводи дословно, но БЕЗ самих знаков ###.
3. Текст в кавычках " генерируй сам и выводи БЕЗ кавычек.
4. Твой ответ должен быть чистым текстом, готовым к публикации.

ЛОГИКА АНАЛИЗА:
- Найди общие Hard Skills, владение ПО и знание ФЗ среди всех кандидатов.
- Обобщи схожие функции в емкие пункты.
- Не выдумывай требования, которых нет в опыте кандидатов.

ШАБЛОН (служебные символы НЕ выводить):

###Вакансия: ### "Название должности"

###ПЕРВОУРАЛЬСКБАНК— динамично развивающийся финансовый институт. Мы предлагаем нашим клиентам современные банковские решения, опираясь на передовые технологии и многолетний опыт работы на финансовом рынке. Мы помогаем нашим партнерам и клиентам решить задачи, связанные с международными платежами. Приглашаем на вакансию целеустремленного и активного### "Краткое вступление о том, кого ищем"

###Обязанности:###
"Список обязанностей (6-8 пунктов), синтезированный из опыта кандидатов."

###Требования:###
"Список требований: стаж, образование, ПО, знание законов."
...
FROM_APPLICATION_VACANCY_PROMPT = """
Ты — старший HR-методолог ПЕРВОУРАЛЬСКБАНКА. Твоя задача: переработать "Заявку на подбор" в описание вакансии.

ПРАВИЛА ОФОРМЛЕНИЯ (КРИТИЧЕСКИ ВАЖНО):
1. СИМВОЛЫ ### и " ВЫВОДИТЬ ЗАПРЕЩЕНО. Твой ответ — чистый текст.
2. Текст между ### выводи дословно, НО без знаков ###.
3. Текст в кавычках " генерируй на основе заявки и выводи БЕЗ кавычек.

ЛОГИКА РАБОТЫ:
- НИЧЕГО НЕ УПУСКАЙ: все требования (ПО, ФЗ, стаж) из заявки должны быть в тексте.
- Группируй мелкие задачи в профессиональные блоки.
- Сохраняй специфику банковской сферы.

ШАБЛОН (служебные символы НЕ выводить):

###Вакансия: ### "Название из заявки"

###ПЕРВОУРАЛЬСКБАНК— динамично развивающийся финансовый институт. Мы предлагаем нашим клиентам современные банковские решения, опираясь на передовые технологии и многолетний опыт работы на финансовом рынке. Мы помогаем нашим партнерам и клиентам решить задачи, связанные с международными платежами. Приглашаем на вакансию целеустремленного и активного### "Описание позиции для привлечения кандидатов"

###Обязанности:###
"Полный список обязанностей из заявки. Можно группировать похожие пункты."

###Требования:###
"Все требования из заявки: стаж, навыки, ПО, законы. Учти СТОП-факторы."
...
def vacancy_type_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="📂 Из заявки Word", callback_data="vac_docx")],
...
@dp.callback_query(F.data == "vac_docx")
async def start_vac_docx(callback: types.CallbackQuery, state: FSMContext):
    await state.set_state(Form.waiting_for_vacancy_docx)
    await state.update_data(vac_method="vac_docx")
    await callback.message.answer("📥 Пожалуйста, пришлите файл заявки в формате **.docx**")
    await callback.answer()
...
async def start_vac_docx(callback: types.CallbackQuery, state: FSMContext):
    await state.set_state(Form.waiting_for_vacancy_docx)
    await callback.message.answer("📥 Пожалуйста, пришлите файл с Заявкой на вакансию в формате .docx", parse_mode="Markdown")
    await callback.answer()

@dp.message(Form.waiting_for_vacancy_docx, F.document)
async def process_vac_docx(message: types.Message, state: FSMContext):
    if not message.document.file_name.lower().endswith(".docx"):
        await message.answer("❌ Ошибка: поддерживаются только файлы .docx"); return

    await message.answer("⌛ Читаю заявку и формирую текст по шаблону ПЕРВОУРАЛЬСКБАНКА...")
    
    try:
        # Скачиваем и парсим Word (используем ваш docx_resume_parser)
        file_content = await bot.download(message.document)
        from docx_resume_parser import extract_resume_data_from_docx
        raw_text = extract_resume_data_from_docx(file_content.read())
        
        # Отправляем в GPT с вашим строгим REVERSE_VACANCY_PROMPT
        res = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": FROM_APPLICATION_VACANCY_PROMPT},
                {"role": "user", "content": f"Сформируй вакансию из этой заявки:\n{raw_text}"}
            ],
            temperature=0.5 # Низкая температура для исключения галлюцинаций
        )
        final_text = res.choices[0].message.content
        
        # Вытаскиваем название (оно в шаблоне после "Вакансия: ")
        title = "Новая вакансия"
        if "Вакансия:" in final_text:
            title = final_text.split("Вакансия:")[1].split("\n")[0].strip()

        await state.update_data(job_title=title, job_text=final_text)
        
        # Показываем результат и кнопку сохранения
        await message.answer(f"<b>📋 Результат:</b>\n\n{final_text}", parse_mode="HTML")
        
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="💾 Сохранить в базу", callback_data="save_vac_confirmed")],
            [InlineKeyboardButton(text="🔄 Сбросить", callback_data="start")]
        ])
        await message.answer(f"Сохранить вакансию '{title}'?", reply_markup=kb)
...
@dp.callback_query(F.data == "save_vac_confirmed")
async def confirm_save_vac(callback: types.CallbackQuery, state: FSMContext):
    data = await state.get_data()
    title, text = data.get("job_title"), data.get("job_text")
    
    db.save_vacancy(title, text)
    await callback.message.edit_text(f"🎯 Вакансия '{title}' успешно сохранена в базу!", reply_markup=main_menu_kb())
    await callback.answer()
...
async def generate_reverse_vac(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        res = client.chat.completions.create(
            ...
            temperature=0.7 # Здесь можно чуть выше, так как нужен творческий синтез
        )
